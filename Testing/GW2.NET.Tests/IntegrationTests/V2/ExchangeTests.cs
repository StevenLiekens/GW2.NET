namespace GW2NET.IntegrationTests.V2
{
    ...
    public class ExchangeTests
    {
        ...
        {
            var result = GW2.V2.Commerce.Exchange.GetGems(coins);
        ...
        {
            var result = await GW2.V2.Commerce.Exchange.GetGemsAsync(coins);
            Assert.NotNull(result);
            Assert.StrictEqual(coins, result.Send);
            Assert.NotInRange(result.Receive, int.MinValue, 0);
            Assert.NotInRange(result.CoinsPerGem, int.MinValue, 0);
        ...
        {
            var result = GW2.V2.Commerce.Exchange.GetCoins(gems);
            Assert.NotNull(result);
        ...
        {
            var result = await GW2.V2.Commerce.Exchange.GetCoinsAsync(gems);
            Assert.NotNull(result);
            Assert.StrictEqual(gems, result.Send);
            Assert.NotInRange(result.Receive, int.MinValue, 0);
            Assert.NotInRange(result.CoinsPerGem, int.MinValue, 0);
