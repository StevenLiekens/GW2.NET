        ...
        public Avatar Read()
        {
            ...
            using (var stream = this.memoryMappedFile.CreateViewStream())
            {
                ...
                try
                {
                    ...
                    avatarDTO = (AvatarDTO)Marshal.PtrToStructure(ptr, typeof(AvatarDTO));
                }
                finally
                {
                    // Release the handle
                ...
                if (!avatarDTO.name.Equals("Guild Wars 2", StringComparison.Ordinal))
                {
                    return null;
                }

                // Convert data contracts to managed data types
                return this.avatarConverter.Convert(avatarDTO, null);
            }
        }
